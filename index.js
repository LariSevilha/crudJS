  ...
  atualizarVetor(id, novoVetor) {
    try {
      ...
      if (index !== -1) {
        ...
        throw new Error(`Registro com ID ${id} não encontrado, portanto nenhum dado foi atualizado.`);
      }
    } catch (error) {
  ...
  deletarVetor(id) {
    try {
      ...
      if (index !== -1) {
        ...
        throw new Error(`Registro com ID ${id} não encontrado, portanto não foi deletado.`);
      }
    } catch (error) {
      console.error(error.message);
    }
